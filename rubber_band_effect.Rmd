...
```{r}
get_transition_counts <- function(data, is_home, poss_interval_length, bin_width) {
  ...
  transitions <- by_int %>%
    ...
    ungroup() %>%
    filter(!is.na(result)) # drops last interval of each game
...
## Analyze rubber band effect for home teams
```{r}
...
```{r}
ggplot(transition_counts_home.10.5, aes(x = score_diff_state, y = prob, fill = result)) +
...
## Analyze rubber band effect for away teams
```{r}
transition_counts_home.10.5 <- get_transition_counts(
  data = pbp,
  is_home = F,
...
```{r}
ggplot(transition_counts_away.10.5, aes(x = score_diff_state, y = prob, fill = result)) +
  geom_col(position = 'fill') +
  labs(
